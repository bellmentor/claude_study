...
- **1회성 작업은 새창(`?tab=`)으로 연다**: 정산마진확인처럼 자주 안 쓰는 1회성 화면은 상단 네비 탭으로 두지 않고 '그때그때' 탭 안에 버튼을 넣어, 클릭 시 `window.open('/?tab=<탭이름>', '_blank')` 로 새 창에서 연다. `app.js` 최상단에서 `location.search` 의 `tab` 쿼리파라미터를 읽어 페이지 로드시 해당 탭을 바로 활성화하는 로직(`activateTab()`)이 이를 지원한다. 탭 버튼 자체는 상단 네비에서 뺀다(정산마진확인 사례, 2026-07-14).
...
## 리차스(사업자) 벤더 관리 (Web UI)

- **리차스**는 `dome_site/`(도매사이트) 나 BearB2B(고도몰 사업자)와 별개로, 매입금을 확인해야 할 사업자이며 그 아래 여러 거래처(벤더)를 둔다. 루트 `lechas/` 폴더 하위에 벤더별 폴더(`lechas/건아유통/`, `lechas/제이원/` 등)를 두고, 그 안에서 각 벤더의 로그인/수집 코드를 개별 작성한다(dome_site 의 `<slug>` 폴더와 같은 성격이나, 사업자 단위로 한 단계 더 묶여있다는 점이 다르다).
- 벤더 목록은 `lechas/벤더_목록.txt` 에서 관리한다 — 한 줄에 벤더명 하나(폴더명과 동일한 한글명 그대로, `도매사이트_폴더_이름.txt` 처럼 영문 slug로 변환하지 않음). `app/server.py` 의 `load_lechas_vendors()` 가 이 파일을 파싱해 Web UI '리차스' 탭 표에 자동 반영한다. 벤더 추가/변경 시 이 파일만 고치면 코드 수정 없이 표에 반영됨.
- Web UI '리차스' 탭(2026-07-14, UI 스캐폴드만 구현됨): 매입금 수집 탭과 동일한 달력(시작~종료일 선택) + 표(체크박스/목록/매입금/상태/상세내용(다운로드)/기타메모). 표의 '상태' 컬럼은 작업 성공/오류 표시용으로 추가됐으나 아직 값이 채워지지 않는다. '수집시작'·'다운로드' 버튼은 현재 실제 수집 로직 없이 "아직 구현되지 않은 기능입니다" 안내만 띄운다 — 각 벤더 폴더 안에 실제 로그인/수집/다운로드 코드가 구현되면 이 버튼들을 연결한다.
