...
- **Git 작업은 사용자 주도가 원칙**: 별도 지시가 없으면 `git add`, `git commit`, `git push`, `git reset` 등 어떤 git 명령도 자동 실행하지 않는다. 사용자가 명시적으로 "git push 해" 또는 그에 준하는 지시를 내렸을 때에 한해 add → commit → push 흐름을 자동으로 처리한다. 그 외의 변경 사항 저장/동기화는 사용자가 직접 한다.
