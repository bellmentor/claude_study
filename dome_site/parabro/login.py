...
LOGIN_URL = "https://www.parabro.co.kr/member/login.php"
SITE_LABEL = "파라브로"
...
async def login() -> bool:
    """파라브로에 로그인한다. 모듈 세션을 재사용하며 종료하지 않는다. 성공 여부 반환."""
    ...
    await page.fill("#loginPwd", pw)

    log.debug("로그인 버튼 클릭")
    login_btn = page.locator("button[type=submit]:has-text('로그인')").first
    # 파라브로 로그인은 폼 submit 으로 페이지가 이동한다. 이동을 기다리되,
    # AJAX 로그인일 가능성에 대비해 실패 시 networkidle 로 폴백한다.
    try:
