"""코코리아(바나나하우스) 매입금 합산 → 도매_매입금.xlsx 에 기록.

계산 규칙 (프롬프트용.txt 기준):
  - '날짜' 에서 해당 월 주문만 남긴다.
  - 각 주문당 (판매가 + 배송비 3,000원) × 1.1(부가세) 를 계산한다.
  - 모두 합하면 해당 월의 매입금.
"""
...
def _to_int(value) -> int:
    """'6,200원' / 6200.0 등을 정수로 변환. 실패 시 0. (소수점 뒤는 버려 10배 방지)"""
    if pd.isna(value):
        return 0
...
def _yyyymm(value) -> str:
    """날짜류 값에서 앞 6자리 숫자(YYYYMM)를 추출. 예: '2026-06-17' → '202606'."""
...
def summarize_purchase(start_date: str) -> int:
    ...
    if "날짜" not in df.columns or "판매가" not in df.columns:
        raise RuntimeError(f"컬럼(날짜/판매가)을 찾지 못했습니다. 컬럼: {list(df.columns)}")
    ...
    # 각 주문당 (판매가 + 배송비) × 1.1, 반올림 후 합산
    total = 0
    for price in df["판매가"]:
        val = (_to_int(price) + SHIPPING_FEE) * VAT
        total += int(round(val))
    log.info(f"매입금(주문당 (판매가+{SHIPPING_FEE})×{VAT} 합) = {total:,}원 ({len(df)}건)")
