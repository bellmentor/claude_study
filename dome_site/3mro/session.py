"""3MRO Playwright 세션 관리.

같은 도매처(3MRO) 안의 모든 오퍼레이션은 이 모듈에 보관된 단일 브라우저 세션을 공유한다.
세션을 중간에 닫고 다시 열지 않는다 (로그인 쿠키 유실 방지).
다른 도매처로 전환할 때만 `close_session()` 을 호출한다.

※ 3MRO 는 엑셀 다운로드가 새 창(팝업)으로 뜨므로 `accept_downloads=True` 가 필요하다.
"""
...
async def open_session() -> Page:
    ...
    context: BrowserContext = await browser.new_context(
        user_agent=_USER_AGENT, accept_downloads=True
    )
...
async def get_context() -> BrowserContext:
    """살아있는 BrowserContext 를 반환한다. 팝업(새 창) 대기 등에 사용."""
    ctx: BrowserContext | None = _state["context"]
    if ctx is None:
        await open_session()
        ctx = _state["context"]
    return ctx
