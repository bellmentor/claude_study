...
LOGIN_URL = "https://comsmart.co.kr/cmart/bbs/login.php?url=%2Fcmart%2F"
SITE_LABEL = "컴스마트"
...
async def login() -> bool:
    """컴스마트에 로그인한다. 모듈 세션을 재사용하며 종료하지 않는다. 성공 여부 반환."""
    ...
    await page.fill("#login_pw", pw)

    log.debug("로그인 버튼(이미지) 클릭")
    login_btn = page.locator("input[type=image]").first
    try:
