"""젠트레이드 Playwright 세션 관리.

같은 도매처(젠트레이드) 안의 모든 오퍼레이션은 이 모듈에 보관된 단일 브라우저 세션을 공유한다.
...
# "debug" = 브라우저 창 표시, "release" = headless
MODE = "debug"
...
async def open_session() -> Page:
    """브라우저 세션을 시작하고 Page를 반환한다. 이미 열려있으면 기존 Page 반환."""
...
async def get_page() -> Page:
    """살아있는 Page를 반환한다. 세션이 없으면 자동으로 새로 연다."""
    page: Page | None = _state["page"]
    if page is None:
